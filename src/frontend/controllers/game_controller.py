...
class GameController:
    ...
    def run_file_mode(self, file_path: Path, chunk_size: int = 4096) -> bool:
        """
        File upload mode - upload and play game from file.

        Joins game as single player, starts it, uploads file, then waits
        for server to execute all moves before returning control.
        ...
        # Step 1: Join as single player if not already joined
        if self._context.state == ClientState.CONNECTED:
            self._logger_.info("Auto-joining as single player for file playback")
            if not self.send_join_single_player_and_wait():
                return False

        # Step 2: Start the game if not already started
        if self._context.state == ClientState.JOINED:
            if not self.send_start_and_wait():
                return False

        # Step 3: Upload the file
        self.upload_file(file_path, chunk_size)
    ...
    def upload_file(self, file_path: Path, chunk_size: int = 4096) -> bool:
        """
    ...
    def send_join(self, color: str):
        ...
        self._session.send(
            {"command": "join_game", "single_player": False, "color": color}
        )
    ...
    def send_join_single_player(self):
        ...
        self._session.send({"command": "join_game", "single_player": True, "color": ""})
        self._logger_.info(f"Sent join command for single player")

    def send_join_single_player_and_wait(self):
        """Send join game command for single player mode and wait until connection (or timeout)"""
        ...
        timeout = 5.0
        elapsed = 0.0
        while self._context.state != ClientState.JOINED and elapsed < timeout:
            time.sleep(0.1)
            elapsed += 0.1

        if self._context.state != ClientState.JOINED:
            self._logger_.error("Failed to join game - cannot upload file")
            return False
    ...
    def send_start(self):
        ...
        if not self._model.both_players_joined:
            self._console_view.display_warning(
                "Cannot start - waiting for both players"
            )
            return

        self._session.send({"command": "start_game"})
        self._logger_.info("Sent start game command")

    def send_start_and_wait(self):
        """Send start game command and wait until the game has started (or a timeout)"""

        self._logger_.info("Auto-starting game for file playback")
        self.send_start()

        # Wait for game to start
        timeout = 5.0
        elapsed = 0.0
        while self._context.state != ClientState.PLAYING and elapsed < timeout:
            time.sleep(0.1)
            elapsed += 0.1

        if self._context.state != ClientState.PLAYING:
            self._logger_.error("Failed to start game - cannot upload file")
            return False
    ...
    def send_move(self, move: str):
        ...
        self._session.send({"command": "make_move", "move": move})
    ...
    def _handle_menu_choice(self, choice: tuple[str, ...]):
        ...
        try:
            ...
            if state == ClientState.CONNECTED:
                ...
                if choice[0] in actions:
                    ...
                    if action_type == "single":
                        self.send_join_single_player()
                        self._console_view.display_info(
                            "Single-player mode: You control both sides"
                        )
            ...
            # STATE: GAME_OVER
                ...
                self._console_view.display_info(
                    "Game over. Type '1' to restart or 'q' to exit."
                )
                if choice[0] == "1":
                    self.send_end_game()
