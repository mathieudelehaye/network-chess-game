...
class GuiView(IView):
        ...
        try:
            ...
            for symbol, (col, row) in piece_positions.items():
                ...
                scaled_piece = pygame.transform.smoothscale(
                    piece_surface, (self.SQUARE_SIZE - 10, self.SQUARE_SIZE - 10)
                )
    ...
    def display_board(self, fen: str = None, last_move: Optional[str] = None) -> None:
        ...
        if not self._initialised:
            try:
                ...
                self.logger_.error(
                    f"Cannot display board - pygame initialisation failed: {e}"
                )
    ...
    def _render(
        self, show_buttons: bool = False, is_single_player: bool = False
    ) -> None:
    ...
    def _draw_info_panel(
        ...
        if show_buttons:
            ...
            if is_single_player:
                ...
                self._draw_button(
                    self.restart_button_rect, "Restart", pygame.mouse.get_pos()
                )
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if (
                        is_single_player
                        and self.restart_button_rect
                        and self.restart_button_rect.collidepoint(x, y)
                    ):
                    ...
                    if self.quit_button_rect and self.quit_button_rect.collidepoint(
                        x, y
                    ):
                    ...
                    if x < self.BOARD_SIZE and y < self.BOARD_SIZE:
                        ...
                        if from_square is None:
                            ...
                            piece = (
                                self.board_state[row][col] if self.board_state else None
                            )
                            if piece:
                                ...
                                self.logger_.info(
                                    f"Piece {piece_name} picked from square {from_square}"
                                )
                                print(
